...
let fetchCV = (config) => {
    return new Promise((resolve, reject) => {            
        ...
        if (cookie === undefined) {
            ...
            window.open(config.authUrl).focus();
...
(function () {
    ...
    let config = {
        ...
        receiveOnly: beacon.attr('data-receive_only') || undefined,
    };

    // Open AF login page if AMV_SSO_COOKIE is not set
    let openAfLogin = config => window.location.href = config.authUrl;
    ...
    let gravityButton = $('<input type="button"/>')
        ...
        .one('click', (e) => { openAfLogin(config); });
